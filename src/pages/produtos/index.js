...
export default function Produtos(){
    ...
    const editar = (e) => {
        setModoEdicao(true);
        let produtoPraEditar = produtos.find(p => p.id == e.target.id);
        produtoPraEditar.dataCadastro = produtoPraEditar.dataCadastro.substring(0, 10);

        setProduto(produtoPraEditar);
    }
    ...
    const atualizarProdutoNoBackend = (produto) => {
        ...
        .then(response => {
            ...
            let indice = produtos.findIndex(p => p.id == produtos.id);
    ...
    return (
        ...
        <div className="row">
            <div className="col-sm-12">
                <button onClick={adicionar} id="btn-adicionar" className="btn btn-warning btn-sm" 
                    data-bs-toggle="modal" data-bs-target="#modal-produto">Adicionar</button>
            </div>
        </div>
                    ...
                    <tbody>
                         {produtos.map(produto => (
                             <tr>
                                ...
                                <td className="botoes">
                                    <button id={produto.id} onClick={editar} className="btn btn-warning btn-sm mr-3" 
                                        data-bs-toggle="modal" data-bs-target="#modal-produto"> Editar
                                    </button>
                                    <button id={produto.id} onClick={excluir} className="btn btn-danger btn-sm mr-3">
                                        Excluir
                                    </button>
                                </td>
                            </tr>
            ...
            <div className="modal-dialog">
                <div className="modal-content">
                    <div className="modal-header">
                        <h4 id="titulo-modal" className="modal-title">{modoEdicao ? 'Editar produto' : 'Adicionar produto'}</h4>
                    ...
                    <div className="modal-body">
                        ...
                        <div className="row">
                            ...
                            <div className="col-sm-7">
                                <label for="quantidadeEstoque" className="form-label">Quantidade Estoque</label>
                    ...
                    <div className="modal-footer">
                        <button onClick={salvar} id="btn-salvar" type="button" 
                            className="btn btn-warning btn-sm">Salvar</button>
                        <button id="btn-cancelar" type="button" 
                            className="btn btn-light btn-sm"data-bs-dismiss="modal">Cancelar</button>
                    </div>
                </div>
            </div>
        </div>
